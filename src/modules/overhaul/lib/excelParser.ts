...
//
// 원본: legacy/plantsync/src/lib/excelParser.js (동작을 바꾸지 않고 타입만 입혔다)
// 실제 한국식 설계내역서(64개 시트, 자간 공백 헤더, 원가/집계 시트 혼재)를 처리한다.
...
export interface ParsedTask {
  /** 저장 전 화면에서 쓰는 임시 식별자. DB에 넣을 때는 uuid가 따로 생긴다 */
  id?: string;
  /** 기계 / 전기 / 제어 / 미분류 */
  field: string;
...
const COLUMN_PATTERNS = {
  ...
  plandate: [
    ...
    "plannedstart", "plandate", "schedule",
  ],
} as const;

type ColumnKey = keyof typeof COLUMN_PATTERNS | "name";
...
const HEADER_CELL_KEYWORDS = ["명칭", "품명", "규격", "수량", "단위", "금액", "단가", "비고", "항목", "unit", "qty", "spec", "tagno", "대상설비", "range"];
...
const EQUIP_TO_FIELD: Record<string, string> = {
...
// 4. 설비 자동 분류 (PRD 6.7)
// ---------------------------------------------------------------------------
const EQUIPMENT_RULES: { equipment: string; keywords: string[] }[] = [
...
// helpers
// ---------------------------------------------------------------------------
function sheetHint(sheetName: string): string {
  const m = sheetName.match(/\(([^)]+)\)/);
  return m ? m[1] : sheetName;
}

function classifyEquipment(texts: Cell[]): string | null {
...
function classifyField(equipment: string | null, texts: Cell[]): string | null {
...
function excelSerialToYMD(n: number): string {
  const dt = new Date(Date.UTC(1899, 11, 30) + Math.round(n) * 86400000);
  return `${dt.getUTCFullYear()}-${pad2(dt.getUTCMonth() + 1)}-${pad2(dt.getUTCDate())}`;
}

function parsePlanDates(cell: Cell): { start: string; end: string | null } | null {
...
function parseOneDate(cell: Cell): string | null {
  if (cell == null || cell === "") return null;
...
function isIndexToken(v: Cell): boolean {
  const s = norm(v);
...
function mapColumns(headerRow: Row): ColumnMap {
  const map: ColumnMap = {};
  headerRow.forEach((cell, idx) => {
    const t = squash(cell);
    if (!t) return;
    for (const [field, patterns] of Object.entries(COLUMN_PATTERNS)) {
      const key = field as keyof typeof COLUMN_PATTERNS;
      if (map[key] != null) continue;
      if ((patterns as readonly string[]).some((p) => t === p || t.includes(p))) {
        map[key] = idx;
        break;
      }
    }
...
/**
 * 헤더 후보 행에 명칭류 컬럼이 있는지 —
 * 단가/금액만 있는 2행 헤더의 하위행을 헤더로 오인하지 않도록.
 */
function hasNameHeader(row: Row): boolean {
  return row.some((c) => {
    const t = squash(c);
    if (!t) return false;
    return (
      NAME_STRONG.some((p) => t === p || t.includes(p)) ||
      NAME_WEAK.some((p) => t === p || t.includes(p))
    );
  });
}
...
function isExcludedRow(name: string, qty: number | null, unit: string, allText: string): boolean {
  if (!name) return true;
  const n = squash(name);
  // 노트/수식/기준 설명 행
  ...
  if (qty == null && !unit) return true;
  // 금액만 있는 행
  if (qty == null && !unit && AMOUNT_KEYWORDS.some((a) => allText.includes(a))) return true;
  return false;
}
...
function analyzeSheet(
  ...
  rows: Row[],
  fileName: string,
): { tasks: ParsedTask[]; excludedCount: number } {
  const result: { tasks: ParsedTask[]; excludedCount: number } = { tasks: [], excludedCount: 0 };
  ...
  for (let r = headerIdx + 1; r < rows.length; r++) {
    ...
    if (isExcludedRow(name, qty, unit, allText)) {
...
function dedupe(tasks: ParsedTask[]): ParsedTask[] {
  ...
  for (const t of tasks) {
    ...
    if (at != null) {
      ...
      const prev = out[at];
      if (prev.qty === 0 && t.qty > 0) out[at] = t;
